...
def run_gather_ap(device, data_wrapper_images, prefix_results, prefix_output):
    ...
    for i in range(epoch_logger_train.epoch_index):
        #[ap, all_tp, all_fp]
        run_epoch(epoch_logger_ap, batch_size, num_validation_batches, yolo, data_wrapper_images, device, prefix_output)

    
def run_epoch(epoch_logger_ap, batch_size, num_batches, yolo, data_wrapper_images, device, prefix_output):
    #start of a new epoch  
    epoch_logger_ap.start_epoch()
    print("start epoch index: ", epoch_logger_ap.epoch_index)
...
if __name__ == "__main__":  
    ...
    if not (NAME is None):
        ...
        prefix_output = "saved_output/" + NAME + "/"
