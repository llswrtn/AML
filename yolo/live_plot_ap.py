...
def load(prefix_results):
    ...
    with open(prefix_results+"epoch_logger_ap.pt", "rb") as file:
...
def update_plot(epoch_logger_ap, old_epoch_logger_ap, fig, axs):
    ...
    axs[0, 0].plot(epoch_logger_ap.list_ap, COLOR_VALIDATE, label="validate")
    axs[0, 0].legend(loc="best")

    axs[0, 1].set(title="tp", xlabel="epoch", ylabel="")
    axs[0, 1].plot(epoch_logger_ap.list_tp, COLOR_VALIDATE, label="validate")
    axs[0, 1].legend(loc="best")

    axs[0, 2].set(title="class accuracy", xlabel="epoch", ylabel="")
    axs[0, 2].plot(epoch_logger_ap.list_predicted_correct, COLOR_VALIDATE, label="validate")
    ...
    axs[1, 1].plot(epoch_logger_ap.list_fp, COLOR_VALIDATE, label="validate")
    ...
    axs[1, 2].plot(epoch_logger_ap.list_predicted_class_0, COLOR_LABEL_0, label="0")
    axs[1, 2].plot(epoch_logger_ap.list_predicted_class_1, COLOR_LABEL_1, label="1")
    axs[1, 2].plot(epoch_logger_ap.list_predicted_class_2, COLOR_LABEL_2, label="2")
    axs[1, 2].plot(epoch_logger_ap.list_predicted_class_3, COLOR_LABEL_3, label="3")
