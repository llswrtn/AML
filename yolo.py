...
class Yolo(BasicNetwork):
    ...
    Neural Network test
    """
    def __init__(self, imgSize, numChannels, number_of_classes=4, boxes_per_cell=2, dropout_p=0.5, use_sigmoid=True, use_clamp_coordinates=False):
        ...
        #region COLUMN 6
        #Conn Layer
        convSize = int(imgSize / 64)
        self.layer_28_full = nn.Linear(convSize*convSize*1024, 4069)
        #endregion
    ...
    def forward(self, x):     
        ...
        x = T.sigmoid(self.layer_30_full(x)) if self.use_sigmoid else self.layer_30_full(x)
